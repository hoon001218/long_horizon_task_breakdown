...
# LLM backend. MODE is the public switch shared with the web app:
# MODE=chatgpt uses the ChatGPT/OpenAI API, MODE=ollama uses local Ollama.
# SEMANTIC_BACKEND is kept only as a legacy fallback when MODE is absent.
...
RAW_MODE = (os.getenv("MODE") or os.getenv("SEMANTIC_BACKEND") or "auto").strip()
MODE = normalize_llm_backend(RAW_MODE)
LLM_BACKEND = MODE
...
OLLAMA_SHOW_URL = f"{OLLAMA_URL.rsplit('/api/chat', 1)[0]}/api/show"
LLM_TEMPERATURE = 0.0
LLM_TIMEOUT_SEC = 120
...
SETTLE_SEC = 0.7
PRIMITIVE_ACTION_INTERVAL_SEC = 0.8
LIFT_DELTA_M = 0.12
...
def get_ollama_model_capabilities(model_name: str) -> list[str]:
    cached = _OLLAMA_MODEL_CAPABILITIES_CACHE.get(model_name)
    if cached is not None:
        return cached

    payload = {"model": model_name}
    response_payload = post_json(OLLAMA_SHOW_URL, payload, {})
    capabilities = response_payload.get("capabilities")
    normalized = (
        [str(capability).strip().lower() for capability in capabilities]
        if isinstance(capabilities, list)
        else []
    )
    _OLLAMA_MODEL_CAPABILITIES_CACHE[model_name] = normalized
    return normalized


def ensure_ollama_multimodal_available() -> None:
    if not SEND_IMAGE_TO_LLM:
        return
    capabilities = get_ollama_model_capabilities(OLLAMA_MODEL)
    if "vision" not in capabilities:
        capability_text = ", ".join(capabilities) if capabilities else "none reported"
        raise RuntimeError(
            "MODE=ollama requires an Ollama model with vision capability because "
            "SEND_IMAGE_TO_LLM=True. "
            f"OLLAMA_MODEL={OLLAMA_MODEL!r}, capabilities={capability_text}. "
            "Use a vision-capable Ollama model or set SEND_IMAGE_TO_LLM=False if "
            "you intentionally want text-only planning."
        )
...
class LLMSemanticPlanner:
    ...
    def _call_ollama(
        ...
        if SEND_IMAGE_TO_LLM and image_bytes:
            ensure_ollama_multimodal_available()
        system, user = self._prompt(command, world_model, feedback)
...
class Executor:
    ...
    def execute(self, decision: PrimitiveDecision) -> ExecutionResult:
        ...
        self.memory.append_result(result)
        if service_dispatched:
            self._primitive_action_interval()
        return result
    ...
    def _primitive_action_interval(self) -> None:
        if PRIMITIVE_ACTION_INTERVAL_SEC <= 0.0:
            return
        self._settle(PRIMITIVE_ACTION_INTERVAL_SEC)
...
def run() -> int:
    ...
    reporter.status(
        "Startup",
        {
            ...
            "primitive_action_interval_sec": PRIMITIVE_ACTION_INTERVAL_SEC,
            "max_steps": MAX_STEPS,
        },
    )

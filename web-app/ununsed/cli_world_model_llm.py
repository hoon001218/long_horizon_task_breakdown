...
- The CLI validates semantic plans, decomposes them into Moving/Grip/Realease/Homing,
  executes them through ROS2 services, and verifies results with fresh marker/image state.
...
class WorldModelBuilder:
    ...
    def _current_region(self, obj: dict[str, Any], regions: dict[str, Any]) -> str:
        ...
        for region_name, region in regions.items():
            if region.get("type") == "goal" and point_in_box_region(position, region, margin=0.0):
                return region_name
...
class LLMSemanticPlanner:
    ...
    def _prompt(self, command: str, world_model: dict[str, Any], feedback: str) -> tuple[str, str]:
        system = (
            ...
            "   }\n"
            "2) reject_task\n"
            "   Use this when the command is unsafe, impossible, ambiguous, references unknown objects/regions, or asks to move outside the table.\n"
            ...
            "- If one robot cannot reach both source and destination, allow handover; runtime will compute it.\n"
        ...
        user = json.dumps(
            {
                ...
                "required_response": "Return one JSON object using the output schema. Prefer reject_task over unsafe guessing.",
            },
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
...
class SemanticRuntime:
    ...
    def _object_satisfies_destination(self, obj: dict[str, Any], destination: dict[str, Any], world: dict[str, Any]) -> bool:
        dest_type = destination["type"]
        if dest_type == "goal":
            return obj.get("current_region") == f"{destination['goal_color']}_goal"
    ...
    def _select_drop_pose(self, obj: dict[str, Any], destination: dict[str, Any], world: dict[str, Any]) -> dict[str, Any] | None:
        dest_type = destination["type"]
        if dest_type == "goal":
            key = f"{destination['goal_color']}_goal"
        elif dest_type in {"outside_goals", "safe_free_space", "table_center", "handover_buffer"}:
            key = dest_type
        else:
            return None
        candidates = world.get("safe_pose_candidates", {}).get(key, [])
        if not candidates:
            return None
        # Prefer reachable candidate with best clearance and short travel.
        ...
        for candidate in candidates:
            ...
            if reachable_count <= 0:
                continue
            score = -reachable_count + 0.1 * xy_distance(obj["pose"]["position"], candidate["position"])
...
class Executor:
    def __init__(self, node: RosWorldNode, memory: ExecutionMemory, builder: WorldModelBuilder, reporter: Reporter) -> None:
        self.node = node
        self.memory = memory
...
class FinishVerifier:
    def __init__(self, node: RosWorldNode, memory: ExecutionMemory, builder: WorldModelBuilder, runtime: SemanticRuntime) -> None:
        self.node = node
        self.memory = memory
        self.builder = builder
    ...
    def is_complete(self) -> tuple[bool, str]:
        ...
        return True, f"finish verified by {FINISH_VERIFY_CHECKS} fresh marker/image check(s): {last_message}"
    ...
    for condition in conditions:
        for object_id in condition.object_ids:
            ...
            if condition.condition_type == "objects_in_goal":
                if obj.get("current_region") != condition.required_region:
                    return False, f"object {object_id} is in {obj.get('current_region')}, expected {condition.required_region}"
    ...
    for attempt in range(1, MAX_LLM_REPLAN_ATTEMPTS + 1):
        ...
        try:
            ...
            current_feedback = (
                ...
                "If the user command is unsafe or impossible, use reject_task."
            )
...
def run() -> int:
    ...
    executor = Executor(node, memory, builder, reporter)
    finish_verifier = FinishVerifier(node, memory, builder, runtime)

...
class ExecutionMemory:
    ...
    def all_robots_homed(self) -> bool:
        return all(bool(self.robots_homed.get(robot_id, False)) for robot_id in CONTROL_SERVICE_TOPICS)

    def robots_not_homed(self) -> list[str]:
        return [robot_id for robot_id in FINAL_HOMING_ROBOT_ORDER if not self.robots_homed.get(robot_id, False)]

    def failure_key(self, decision: PrimitiveDecision, failure_type: str | None = None) -> str:
        metadata = decision.metadata or {}
        verify = metadata.get("verify_after_action") or {}
        destination = verify.get("region") or verify.get("stage") or verify.get("type") or "unknown_destination"
    ...
    def record_task_failure(self, decision: PrimitiveDecision, failure_type: str | None = None) -> int:
    ...
    def reset_task_failure(self, decision: PrimitiveDecision, failure_type: str | None = None) -> None:
        key = self.failure_key(decision, failure_type)
...
class SemanticRuntime:
    ...
    def load_plan(self, plan: LLMPlan, world: dict[str, Any]) -> None:
        ...
        self.concrete_goal_conditions = self._derive_goal_conditions_from_locked_scope(world)
    ...
    def _derive_goal_conditions_from_locked_scope(self, world: dict[str, Any]) -> list[SemanticGoalCondition]:
        ...
        for object_id, destination in self.locked_object_destinations.items():
            ...
            if dest_type == "goal":
                ...
                grouped.setdefault(("objects_in_goal", key_region), []).append(object_id)
            elif dest_type in {"outside_goals", "safe_free_space"}:
                grouped.setdefault(("objects_outside_goals", "outside_goals"), []).append(object_id)
            elif dest_type in {"table_center", "handover_buffer"}:
                grouped.setdefault(("objects_near_region_pose", dest_type), []).append(object_id)
        for (condition_type, region), object_ids in grouped.items():
            conditions.append(
                SemanticGoalCondition(
                    ...
                    required_goal_color=(region.split("_", 1)[0] if region and region.endswith("_goal") else None),
        ...
        if self.locked_object_destinations:
            ...
            for object_id, destination in self.locked_object_destinations.items():
                ...
                call = self.locked_source_tool_calls.get(object_id) or SemanticToolCall(
                    "move_objects",
                    {"object_selector": {"ids": [object_id]}, "destination": destination, "handover_policy": "use_if_needed"},
    ...
    def _safe_pre_pick_sequence(
        ...
        if not SAFE_PRE_PICK_ENABLED:
            return [self._decision(robot_id, ACTION_MOVING, object_pose, obj["id"], "pick", pick_reason, task)]
        ...
        decisions.append(self._decision(robot_id, ACTION_MOVING, object_pose, obj["id"], "pick", pick_reason, task))
        return decisions
    ...
    def _decompose_direct(self, world: dict[str, Any], obj: dict[str, Any], route: RouteCandidate, task: ObjectTask) -> list[PrimitiveDecision]:
        ...
        if obj.get("held_by") != robot_id and self.memory.held_objects.get(robot_id) != obj["id"]:
            decisions.extend(self._safe_pre_pick_sequence(world, robot_id, obj, task, "Move to the selected object pose."))
            decisions.append(self._decision(robot_id, ACTION_GRIP, None, obj["id"], "grip", "Grip the selected object.", task))
        decisions.extend(
            [
    ...
    def _decompose_handover(self, world: dict[str, Any], obj: dict[str, Any], route: RouteCandidate, task: ObjectTask) -> list[PrimitiveDecision]:
        ...
        decisions: list[PrimitiveDecision] = []
        decisions.extend(self._safe_pre_pick_sequence(world, src, obj, task, "Source robot moves to object."))
        decisions.extend(
            [
        ...
        decisions.extend(self._safe_pre_pick_sequence(world, dst, buffer_obj, task, "Destination robot moves to handover buffer object."))
        decisions.extend(
            [
...
class Executor:
    ...
    def _reconcile_failed_action(self, decision: PrimitiveDecision, message: str) -> None:
        ...
        if not object_id or robot_id not in self.memory.held_objects:
            return
        if decision.action == ACTION_MOVING and decision.intent == "lift" and self.memory.pending_grip_targets.get(robot_id) == object_id:
        ...
        verify = decision.metadata.get("verify_after_action") if decision.metadata else {}
        ...
        if eef_dist > GRASP_FALSE_HELD_EEF_DISTANCE_M and target_dist > VERIFY_XY_TOLERANCE_M:
            self.memory.held_objects[robot_id] = None
            self.memory.pending_grip_targets[robot_id] = None
            self.memory.last_verification = {
                "ok": False,
    ...
    def _needs_grasp_follow_verification(self, decision: PrimitiveDecision) -> bool:
        ...
        return bool(target_id and self.memory.pending_grip_targets.get(robot_id) == target_id)

    def _verify_grasp_following_eef(self, world: dict[str, Any], decision: PrimitiveDecision) -> tuple[bool, str]:
        robot_id = str(decision.robot_id)
        ...
        eef_pose = robot.get("end_effector_pose")
        if obj is None:
            return False, f"grasp confirmation failed: object {object_id} is not visible after lift"
        if not eef_pose or not eef_pose.get("position"):
            return False, f"grasp confirmation failed: EEF pose for {robot_id} is unavailable"
        obj_pos = obj["pose"]["position"]
        eef_pos = eef_pose["position"]
        ...
        if decision.target_pose and decision.target_pose.get("position"):
            target_z_error = abs(float(obj_pos.get("z", 0.0)) - float(decision.target_pose["position"].get("z", 0.0)))
        ...
        z_ok = z_error <= GRASP_FOLLOW_Z_TOLERANCE_M or target_z_error <= GRASP_FOLLOW_Z_TOLERANCE_M
    ...
    def _update_holding_state(self, decision: PrimitiveDecision, success: bool) -> None:
        ...
        if decision.action == ACTION_MOVING and decision.intent == "pick":
            ...
            self.memory.pending_grip_targets[robot_id] = decision.target_object_id or self.memory.pending_grip_targets.get(robot_id)
        elif decision.action == ACTION_MOVING and decision.intent == "lift":
            if decision.target_object_id and self.memory.pending_grip_targets.get(robot_id) == decision.target_object_id:
...
class FinishVerifier:
    ...
    def is_complete(self) -> tuple[bool, str]:
        ...
        if FINAL_VERIFY_REQUIRES_ALL_ROBOTS_HOMED and not self.memory.all_robots_homed():
            return False, f"final verification requires both robots homed first; not_homed={self.memory.robots_not_homed()}, homed_state={self.memory.robots_homed}"
        ...
        if last_world is not None and last_image_bytes is not None:
            ...
            elif not visual_ok and not FINAL_VISUAL_VERIFICATION_CAN_VETO_MARKER_SUCCESS:
                visual_message = (
        ...
        return True, f"finish verified after both robots homed by {FINISH_VERIFY_CHECKS} fresh marker checks; multimodal visual check was advisory: {last_message}"

...
DEFAULT_BUFFER_CLEARANCE_M = 0.045
...
class TaskSpec:
    """Command-dependent task definition.

    The old planner implicitly assumed red->red and blue->blue.  This structure
    makes the command explicit: which object colors are active, and which goal
    color each active object color must be placed into.
    """

    raw_command: str
    active_object_colors: set[str]
    destination_by_object_color: dict[str, str]

    def is_active_object(self, obj: dict[str, Any]) -> bool:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "destination_by_object_color": dict(
                sorted(self.destination_by_object_color.items())
            ),
        }


class TaskInterpreter:
    """Small rule-based interpreter for color-to-goal commands.

    Supported examples:
    - 빨간 물체는 빨간 목표점에, 파란 물체는 파란 목표점에 놓아라
    - 파란 물체를 전부 빨간 목표점에 넣어라
    - 모든 물체를 빨간 목표점에 넣어라
    - put all blue objects into the red goal

    Unsupported or ambiguous commands are rejected instead of silently falling
    back to the old fixed red->red / blue->blue policy.
    """
    ...
    def parse(self, command: str) -> TaskSpec:
        ...
        if not mentions:
            raise ValueError(
                "Task command does not specify any supported color. Supported colors are red/blue."
            )

        mapping: dict[str, str] = {}

        # Default color sorting command usually appears as red,red,blue,blue.
        ...
        elif has_all and len(mentions) >= 2:
            dst = mentions[-1][1]
            for _, src in mentions[:-1]:
                mapping[src] = dst
        else:
            raise ValueError(
                "Ambiguous task command. Use an explicit form such as "
                "'blue objects to red goal' or 'red objects to red goal, blue objects to blue goal'."
            )

        if not mapping:
            raise ValueError(
                "No executable color-to-goal mapping could be extracted from the command."
            )

        unsupported = [
            color
            for color in set(mapping) | set(mapping.values())
            if color not in self.COLOR_PATTERNS
        ]
        if unsupported:
            raise ValueError(
                f"Unsupported color(s) in command: {unsupported}. Supported colors are red/blue."
            )

        return TaskSpec(
            raw_command=command,
            active_object_colors=set(mapping.keys()),
            destination_by_object_color=mapping,
        )
    ...
    def _color_mentions(self, text: str) -> list[tuple[int, str]]:
        ...
        # Deduplicate near-identical overlapping mentions produced by Korean stems.
...
def object_in_destination_goal(
    obj: dict[str, Any], goals: dict[str, Any], task_spec: TaskSpec
) -> bool:
    destination = task_spec.destination_for(obj)
    if destination is None:
        return True
    goal = goals.get(destination)
    if goal is None:
        return False
    return point_in_goal(obj["pose"]["position"], goal, margin=0.0)
...
def task_complete_by_vision(
    ...
    if pending_grip_targets and any(
        value is not None for value in pending_grip_targets.values()
    ):
        return False, f"pending grip target(s) remain: {pending_grip_targets}"

    goals = observation.get("goals", {})
    missing_goals = sorted(
        set(task_spec.destination_by_object_color.values()) - set(goals.keys())
    )
    if missing_goals:
        return False, f"required goal marker(s) missing: {missing_goals}"

    active_objects = [
        obj for obj in observation.get("objects", []) if task_spec.is_active_object(obj)
    ]
    if not active_objects:
        return (
            False,
            f"no visible active objects for colors {sorted(task_spec.active_object_colors)}",
        )

    failures: list[str] = []
    for obj in active_objects:
        dst = task_spec.destination_for(obj)
        goal = goals.get(dst) if dst else None
        inside = goal is not None and point_in_goal(
            obj["pose"]["position"], goal, margin=0.0
        )
        if not inside:
            failures.append(
                f"{obj['id']} color={obj.get('color')} destination={dst} "
    ...
    return (
        True,
        f"all active objects satisfy {task_spec.as_dict()} and no robot is holding an object",
    )


def validate_task_spec_against_scene(
    task_spec: TaskSpec, observation: dict[str, Any]
) -> None:
    ...
    if missing_object_colors:
        raise ValueError(
            f"Command targets object color(s) not visible in the scene: {missing_object_colors}"
        )
    missing_goals = sorted(
        set(task_spec.destination_by_object_color.values())
        - set(observation.get("goals", {}).keys())
    )
    if missing_goals:
        raise ValueError(
            f"Command requires goal color(s) not visible in the scene: {missing_goals}"
        )
...
class HandoverTaskPlanner:
    ...
    def _make_high_level_plan(
        ...
        unsorted = [
            ...
            if self.task_spec.is_active_object(obj)
            and not object_in_destination_goal(
                obj, observation.get("goals", {}), self.task_spec
            )
        ]
    ...
    def _high_level_rationale(
        ...
        destination_color = self.task_spec.destination_for(obj) if obj else None
        ...
        if route.route_type == "direct":
            return (
                ...
                f"can reach both the object and the corrected {destination_color} goal drop pose."
            )
        ...
        def attach(decisions: list[Decision]) -> list[Decision]:
            ...
            for idx, decision in enumerate(decisions, start=1):
                ...
                if isinstance(verify_spec, dict):
                    verify_spec.setdefault(
                        "expected_goal_color", common_meta.get("target_goal_color")
                    )
            ...
            return attach(
                [
                    ...
                    self._release(
                        rid,
                        obj["id"],
                        "Release the object inside its command-specified goal.",
            ...
            return attach(
                [
                    ...
                    self._release(
                        rid,
                        obj["id"],
                        "Release the object inside the command-specified goal.",
                        verify_type="final_drop",
                        expected_pose=drop_pose,
                    ),
                    self._homing(
            ...
            return attach(
                [
                    ...
                    self._release(
                        dest,
                        obj["id"],
                        "Destination robot releases the object inside the command-specified goal.",
                        verify_type="final_drop",
                        expected_pose=drop_pose,
                    ),
                    self._homing(
    ...
    def _breakdown_text(self, route: RouteCandidate, obj: dict[str, Any]) -> str:
        if route.route_type == "direct":
            return (
                f"High-level task: move {obj['id']} to the {self.task_spec.destination_for(obj)} goal. "
        ...
        if route.route_type == "handover":
            return (
                f"High-level task: move {obj['id']} to the {self.task_spec.destination_for(obj)} goal through handover. "
...
class Executor:
    ...
    def _check_verification_spec(
        ...
        if verification_type == "final_drop":
            expected_goal_color = str(
                verify_spec.get("expected_goal_color") or obj.get("color")
            )
            goal = observation.get("goals", {}).get(expected_goal_color)
            inside_command_goal = goal is not None and point_in_goal(
                actual_position, goal, margin=0.0
            )
            if inside_command_goal:
                return (
                    True,
                    f"object {object_id} is inside the command-specified {expected_goal_color} goal",
                )
            return False, (
...
def verify_final_completion(
    ...
    for index in range(checks):
        spin_sleep(
            node,
            float(args.verify_delay if index == 0 else args.finish_verify_interval),
        )
...
def resolve_task_command(
...
) -> tuple[str | None, TaskSpec | None]:
    """Resolve and validate the high-level task command.

    Priority:
    1. --input / --command for scripted use.
    2. --use-default-command for non-interactive default execution.
    3. Interactive terminal prompt.

    The prompt validates the command immediately. Unsupported or ambiguous commands
    are rejected before ROS initialization and before any robot motion is attempted.
    """
    ...
    if provided:
        try:
            return provided, interpreter.parse(provided)
        except ValueError as exc:
            reporter.error(f"Rejected task command: {exc}")
            return None, None
    ...
    reporter.info("  3) 모든 물체를 파란 목표점에 넣어라")
    reporter.info("  4) red objects to blue goal, blue objects to red goal")
    reporter.info(
    ...
    while True:
        ...
        try:
            task_spec = interpreter.parse(typed)
        except ValueError as exc:
            reporter.error(f"Rejected task command: {exc}")
            reporter.info(
                "Enter a clearer command, for example: 파란 물체를 전부 빨간 목표점에 넣어라"
            )
            continue

        reporter.status(
            "Accepted task command",
            {"command": typed, "task_spec": task_spec.as_dict()},
        )
        return typed, task_spec
...
def run(args: argparse.Namespace) -> int:
    ...
    resolved_command, task_spec = resolve_task_command(args, reporter)
    if resolved_command is None or task_spec is None:
        return 1
    args.command = resolved_command

    reporter.status(
        "Startup",
        {
            "planner": "command_aware_hierarchical_task_director_with_handover_decomposer",
    ...
    try:
        ...
        initial_observation, _ = node.snapshot()
        try:
        ...
        for step in range(1, int(args.max_steps) + 1):
            ...
            reporter.status(
                "Observation",
                {
                    ...
                    "task_pending": [
                        obj["id"]
                        for obj in observation.get("objects", [])
                        if task_spec.is_active_object(obj)
                        and not object_in_destination_goal(
                            obj, observation.get("goals", {}), task_spec
                        )
                    ],
            ...
            if decision.done:
                ...
                planner.clear_active_plan()
                reporter.error(
                    f"Finish was rejected by fresh vision/marker verification: {detail}"
                )
                if args.once:
                    return 1
                continue

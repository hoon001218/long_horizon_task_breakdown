...
# only after strict marker/geometry checks have passed. If it fails, the action/finish
# is rejected and the planner must continue or replan.
        ...
        return json.dumps(
            {
                ...
                "instruction": (
                    "Verify whether the last robot action truly achieved the predicate. "
                    "Use both the top-view image and the symbolic ROS2 world model. Be conservative: "
                    "for goal placement, the full visible object footprint must be inside the goal area, not merely touching a boundary; "
                    "the object must not be held by a gripper. Return JSON only."
                ),
                "predicate": verify,
        ...
        return json.dumps(
            {
                ...
                "instruction": (
                    ...
                    "Use both the top-view image and the symbolic ROS2 world model. Be conservative: "
                    "objects must be released, not held; objects requested inside a goal must have their full footprint inside that goal, "
                    "not on the border. Return JSON only."
                ),

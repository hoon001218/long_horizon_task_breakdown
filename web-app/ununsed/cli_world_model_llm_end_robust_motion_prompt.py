...
class LLMSemanticPlanner:
    ...
    def _prompt(self, command: str, world_model: dict[str, Any], feedback: str) -> tuple[str, str]:
        system = (
            ...
            "- You still must not output low-level waypoints or ROS primitives. The runtime will compute lift, approach, handover, and safe intermediate poses.\n"
            "Safety rules:\n"
        ...
        user = json.dumps(
            {
                ...
                "required_response": "Return one JSON object using the output schema. Extract semantic intent with move_objects whenever possible. Use reject_task only for unsafe, unknown, or ambiguous commands; never reject because a handover may be required. Remember that point-to-point straight motion can collide with intermediate objects; mention lifted/clearance and collision-aware safe routing in reasoning when relevant, but do not output ROS primitives or raw waypoints.",
            },
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
